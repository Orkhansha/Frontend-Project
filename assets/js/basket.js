...
for (let product of products) {
  tableBody.innerHTML += `<tr>
    <td scope="col" cat-id="${product.id}"><img src="${product.image}" height="100px" width="100px" class="card-img-top" alt="..."></td>
    <td scope="col">${product.name}</td>
    <td scope="col"><span>${product.cost}</span> $</td>
    <td scope="col"><i class="fa-solid fa-minus"></i><span class="Count">${product.count}</span><i class="fa-solid fa-plus"></i></td>
...
document.querySelector("#totalprice").innerHTML=parseFloat(shipping)+parseFloat(gst)+parseFloat(total)
...
for(let i=0;i<inc.length;i++){
  ...
  button.addEventListener("click", function(event){
    ...
    if( numbertd.innerText==newnumber){
      ...
      let newcost=parseFloat(oldcost)+parseFloat(this.parentNode.previousElementSibling.firstElementChild.innerText)
     ...
     var newsub=parseFloat(oldsubtotal)+parseFloat(this.parentNode.previousElementSibling.firstElementChild.innerText)
      oldsub.innerText=newsub
    var oldtotal=totalprice
    var oldtotalprice=oldtotal.innerText
    var newtotal=parseFloat(oldtotalprice)+parseFloat(this.parentNode.previousElementSibling.firstElementChild.innerText)
